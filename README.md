...
- 本地音乐页：搜索歌名、歌手、专辑、歌词和拼音，图标排序、随机播放本地全部歌曲；歌曲卡片显示歌名，以及同一行内的歌手和专辑，宽屏窗口下自动切换多列歌曲列表。
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\windows_installer\build-windows-installer.ps1 -Version 1.2
```

产物：

```text
dist\QingTingSetup-v1_2.exe
```
